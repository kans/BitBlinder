...
class Statistics:
    def __init__(self, upmeasure, downmeasure, connecter, httpdl,
                 ratelimiter, rerequest, fdatflag):
    ...
    def update(self):
        ...
        s.knownSeeds = self.rerequest.knownSeeds
        s.knownPeers = self.rerequest.knownPeers
        s.failureReason = self.rerequest.errorMsg
...
#                    break
        s.numCopies = self.picker.get_num_copies(False)
