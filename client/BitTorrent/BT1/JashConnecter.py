...
class Connection:
    def __init__(self, protocol, connecter, ccount):
    ...
    def get_ip(self):
        return self.protocol.get_ip()

    def get_id(self):
        return self.protocol.get_id()

    def get_readable_id(self):
        return self.protocol.get_readable_id()
        
    def get_client_name(self):
        return self.protocol.get_client_name()

    def close(self):
        log_msg('%s connection closed' % (self.ccount), 3, "btconn")
        self.protocol.close()

    def is_locally_initiated(self):
        return self.protocol.is_locally_initiated()

    def is_encrypted(self):
        return self.protocol.is_encrypted()
    ...
    def send_request(self, index, begin, length):
        ...
        log_msg('%s sent request %s %s %s' % (self.ccount,index,begin,begin+length), 4, "btconn")

    def send_cancel(self, index, begin, length):
        self._send_message(CANCEL + tobinary(index) + 
            tobinary(begin) + tobinary(length))
        log_msg('%s sent cancel %s %s %s' % (self.ccount,index,begin,begin+length), 4, "btconn")
    ...
    def _send_message(self, s):
        if s:
            log_msg("%s SENDING MESSAGE %s %s" % (self.ccount,ord(s[0]),len(s)), 4, "btconn")
        else:
            log_msg("%s SENDING MESSAGE %s %s" % (self.ccount,-1,0), 4, "btprotocol")
        s = tobinary(len(s))+s
        if self.partial_message:
            self.outqueue.append(s)
        else:
            self.protocol.send_message_raw(s)
    ...
    def backlogged(self):
        return not self.protocol.is_flushed()
    ...
    def get_circuit(self):
      circ = None
      try:
        circ = self.protocol.transport.nomnetStream.circuit
      except Exception, e:
        pass
      return circ
...
class JashConnecter:
    ...
    def connection_made(self, protocol):
        self.ccount += 1
        c = Connection(protocol, self, self.ccount)
        log_msg('%s connection made' % (c.ccount), 3, "btprotocol")
        self.connections[protocol] = c
    ...
    def connection_flushed(self, protocol):
    ...
    def got_message(self, protocol, message):
        ...
        if t == CHOKE:
            ...
            if len(message) != 5:
                log_msg('%s bad message length (HAVE)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            i = toint(message[1:])
            if i >= self.numpieces:
                log_msg('%s bad piece number (HAVE)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            ...
            if len(message) != 13:
                log_msg('%s bad message length (REQUEST)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            i = toint(message[1:5])
            if i >= self.numpieces:
                log_msg('%s bad piece number (REQUEST)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            ...
            if len(message) != 13:
                log_msg('%s bad message length (CANCEL)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            i = toint(message[1:5])
            if i >= self.numpieces:
                log_msg('%s bad piece number (CANCEL)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            ...
            if len(message) <= 9:
                log_msg('%s bad message length (PIECE)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
            i = toint(message[1:5])
            if i >= self.numpieces:
                log_msg('%s bad piece number (PIECE)' % (c.ccount), 0, "btprotocol")
                protocol.close()
                return
