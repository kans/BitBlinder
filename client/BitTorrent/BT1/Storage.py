...
from BitTorrent.piecebuffer import BufferPool
...
from common.system import FileLocks
if os.name == 'nt':
  import win32con
            ...
            if length == 0:
                ...
                if disabled_files[i]:
                    ...
                    if exists(file):
                        ...
                        if os.name == 'nt':
                          if len(file) > 259:
                            raise IOError("File names too long.  Save somewhere else (like C:\\temp)")
                        h = open(file, 'wb+')
                        h.flush()
                        h.close()
...
#                               min(MAXLOCKSIZE,self.sizes[name]-p))
        ...
        def _lock_file(self, name, f):
            FileLocks.lock(f, FileLocks.LOCK_EX)
#            import fcntl
...
#            fcntl.flock(f.fileno(), fcntl.LOCK_UN)
    ...
    def close(self):
        for file, f in self.handles.items():
            try:
                ...
                log_ex(e, "Failed to unlock")
            try:
                f.close()
            except Exception, e:
                log_ex(e, "Failed to close")
        self.handles = {}
        self.whandles = {}
        self.handlebuffer = None
    ...
    def delete_file(self, f):
        try:
            if os.path.exists(self.files[f][0]):
              os.remove(self.files[f][0])
        except Exception, e:
            log_ex(e, "Failed to delete")
