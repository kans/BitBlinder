...
#            return
        ...
        self.backfunc(self._initialize, 0.01)
    ...
    def unpickle(self, data, valid_places):
        ...
        try:
            if data['pieces'] == 1:     # a seed
                assert not data.get('places',None), 'data get places'
                assert not data.get('partials',None), 'data get partials'
                have = Bitfield(len(self.hashes))
                for i in xrange(len(self.hashes)):
                    have[i] = True
                assert have.complete(), 'have not complete'
                _places = []
                _partials = []
            else:
                have = Bitfield(len(self.hashes), data['pieces'])
                _places = data['places']
                assert len(_places) % 2 == 0, 'places not even'
                _places = [_places[x:x+2] for x in xrange(0,len(_places),2)]
                _partials = data['partials']
                assert len(_partials) % 2 == 0, 'partials not even'
                _partials = [_partials[x:x+2] for x in xrange(0,len(_partials),2)]
                
            for index, place in _places:
                if place not in valid_places:
                    continue
                assert not got.has_key(index), 'got has index'
                assert not got.has_key(place), 'got has place'
                places[index] = place
                got[index] = 1
                got[place] = 1
                ...
                if have[index]:
                    if not places.has_key(index):
                        if index not in valid_places:
                            have[index] = False
                            continue
                        assert not got.has_key(index), 'got has index 2'
                        places[index] = index
                        got[index] = 1
                    length = self._piecelen(index)
            ...
            for index, plist in _partials:
                assert not dirty.has_key(index), 'dirty has index'
                assert not have[index], 'have index'
                if not places.has_key(index):
                    if index not in valid_places:
                        continue
                    assert not got.has_key(index), 'got has index 3'
                    places[index] = index
                    got[index] = 1
                assert len(plist) % 2 == 0, 'len plist not even'
                ...
                for i in xrange(len(plist)-1):
                    ...
                    assert not end > plist[i+1][0], 'end > plist ?!?!?'
                    l.append((end,plist[i+1][0]-end))
                end = plist[-1][0]+plist[-1][1]
                assert not end > length, 'end > length'
                if end < length:
            ...
            assert amount_obtained + amount_inactive == self.amount_desired, \
              'amount obtained + inactive %s is too much %s' % (amount_obtained + amount_inactive,  self.amount_desired)
        except Exception, e:
            #TODO:  I dont know why this fails so often, but the program seems to cope with it fine
            if type(e) == AssertionError:
              log_msg("Unpickle assertion failed:  %s" % (e.message), 0)
            else:
              log_ex("Unpickle failed", e)
