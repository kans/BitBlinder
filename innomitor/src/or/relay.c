                           ...
                           cell_direction_t cell_direction)
{
  ...
  //Josh: dont bother tracking payments unless we're closing circuits
  if (get_options()->CloseUnpaid) {
  ...
  if (circ->payedReads < -CIRCWINDOW_INCREMENT || circ->payedWrites < -CIRCWINDOW_INCREMENT) {
    log_warn(LD_CIRC, "Josh:  not enough payments during relay cell recv:  read (%d) write (%d)", circ->payedReads, circ->payedWrites);
    return -END_CIRC_REASON_UNPAID;
  }
                             ...
                             size_t payload_len, crypt_path_t *cpath_layer)
{
  ...
  if (circ->payedReads < -CIRCWINDOW_INCREMENT || circ->payedWrites < -CIRCWINDOW_INCREMENT) {
    log_warn(LD_CIRC, "Josh:  not enough payments during relay cell send:  read (%d) write (%d)", circ->payedReads, circ->payedWrites);
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    ...
    if (origin_circ->remaining_relay_early_cells > 0 &&
        //Josh:  added this line:  these dont count as RELAY_EARLY:
        relay_command != RELAY_COMMAND_MESSAGE &&
...
//      log_info(domain,"Got a relay-level padding cell. Dropping.");
    ...
    case RELAY_COMMAND_DATA:
      ...
      if (( layer_hint && --layer_hint->deliver_window < 0) ||
        ...
        //Josh:  TODO:  this is here to prevent a later assert from having a negative deliver_window.  It should not be happening anyway, so figure out why and fix.
        if (layer_hint) {
          layer_hint->deliver_window = 0;
        } else {
          circ->deliver_window = 0;
        }
        connection_edge_end(conn, END_STREAM_REASON_TORPROTOCOL);
        connection_mark_for_close(TO_CONN(conn));
        return -END_CIRC_REASON_TORPROTOCOL;
...
/* XXX add to this log_fn the exit node's nickname? */
    ...
    case RELAY_COMMAND_CONNECTED:
      ...
      return 0;
    case RELAY_COMMAND_MESSAGE:
      if (1) {
        char base64Buffer[1000];
        //have to convert data to base64 to get it through the control interface
        if (base64_encode(base64Buffer, 1000, cell->payload+RELAY_HEADER_SIZE, RELAY_PAYLOAD_SIZE) < 0) {
          log_warn(LD_CONTROL, "Josh:  Unable to format string for controller.");
          return 0;
        }
        tor_strstrip(base64Buffer, "\n");
        
        if (!CIRCUIT_IS_ORIGIN(circ)) {
          control_event_orcircuit("PAYMENT", base64Buffer, circ, CELL_DIRECTION_OUT);
        } else {
          control_event_orcircuit("PAYMENT", base64Buffer, circ, CELL_DIRECTION_IN);
        }
      }
      return 0;
...
{
  //Josh:  DOC
  if (layer_hint ? circ->payedWrites <= layer_hint->package_window : circ->payedReads <= circ->package_window) {
    log_debug(LD_CIRC, "Josh:  did not yet resume edge reading:  read (%d) write (%d)", circ->payedReads, circ->payedWrites);
    return;
  }
  if (layer_hint ? layer_hint->package_window <= 0 : circ->package_window <= 0) {
    log_debug(LD_CIRC, "Josh:  did not yet resume edge reading, package window is too low!");
    return;
  }
...
circuit_consider_stop_edge_reading(circuit_t *circ, crypt_path_t *layer_hint)
{
  ...
  if (!layer_hint) {
    ...
    log_debug(domain,"considering circ->package_window %d",
              circ->package_window);
    if (circ->package_window <= 0 || circ->payedReads <= CIRCWINDOW_INCREMENT) {
      log_debug(domain,"yes, not-at-origin. stopped.");
      log_debug(domain, "Josh:  stopped reading:  read (%d) write (%d)", circ->payedReads, circ->payedWrites);
  ...
  log_debug(domain,"considering layer_hint->package_window %d",
            layer_hint->package_window);
  if (layer_hint->package_window <= 0 || circ->payedWrites <= CIRCWINDOW_INCREMENT) {
...
//         layer_hint ? "defined" : "null");
  int deliver_window;
  deliver_window = (layer_hint ? layer_hint->deliver_window : circ->deliver_window);

  //Josh:  NOTE:  this stops at CIRCWINDOW_INCREMENT because otherwise 
  if (layer_hint ? circ->payedReads <= layer_hint->deliver_window : circ->payedWrites <= circ->deliver_window) {
    log_debug(LD_CIRC, "Josh:  Not sending SENDME:  read (%d) write (%d) deliver_window (%d)", circ->payedReads, circ->payedWrites, deliver_window);
    return;
  }
