...
//Josh:
#define RELAY_COMMAND_MESSAGE 18
...
//Josh:
#define END_STREAM_REASON_UNPAID 16
...
//Josh:
#define END_CIRC_REASON_UNPAID          13
...
typedef struct or_connection_t {
  ...
  //Josh:  MSVC makes enums as signed ints, which makes enums fail
  #ifndef WIN32
  circ_id_type_t circ_id_type:2; /**< When we send CREATE cells along this
                                  * connection, which half of the space should
                                  * we use? */
  #else
  unsigned int circ_id_type : 3;
  #endif
...
typedef struct dir_connection_t {
  ...
  //Josh:  MSVC makes enums as signed ints, which makes enums fail
  #ifndef WIN32
  dir_spool_src_type dir_spool_src : 3;
  #else
  unsigned int dir_spool_src : 3;
  #endif
...
typedef struct addr_policy_t {
  ...
  unsigned int policy_type:2;/**< What to do when the policy matches.*/
...
typedef struct {
  ...
  unsigned int allow_single_hop_exits:1;  /**< Whether the router allows
                                     * single hop exits. */
  unsigned int allow_dht_exit:1; /**< Whether the control will send DHT requests
                                      * on behalf of others or not. */
...
typedef struct circuit_t {
  ...
  //Josh:  these correspond basically to the above variables.  They represent the number of relay data cells that were paid for so far:
  int payedReads;
  int payedWrites;
  //and these are for tracking how they changed, for bandwidth, essentially.
  int payedReadsAdded;
  int payedWritesAdded;
...
typedef struct {
  ...
  //Whether to die unless we receive periodic keepalive events from the controller or not
  int RequiresKeepAlives;
...
#define LOG_PROTOCOL_WARN (get_options()->ProtocolWarnings ? \
  ...
  /** If true, our controller has offered to send DHT requests on behalf of others. */
  int AllowDHTExits;
  /** If true, close any circuits that do not send enough PAR cells for a circuit. */
  int CloseUnpaid;
...
//Josh:  added this function back in:
...
int control_event_circuit_token_levels(void);
...
void control_event_orcircuit(const char* msgType, const char *data, circuit_t* circ, int direction);
