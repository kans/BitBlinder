...
/** What was the read bucket before the last second_elapsed_callback() call?
 * (used to determine how many bytes we've read). */
static int stats_prev_relayed_read_bucket;
/** What was the write bucket before the last second_elapsed_callback() call?
 * (used to determine how many bytes we've written). */
static int stats_prev_relayed_write_bucket;
...
/** When did we last process a KEEPALIVE event from the controller? */
time_t time_to_die;
...
run_scheduled_events(time_t now)
{
  ...
  if (options->RequiresKeepAlives) {
    //Set the time if it has not already been set
    if (time_to_die == 0) {
      time_to_die = now + 120;
    }
    //Josh:  see if we should end ourselves because we have not received a KEEPALIVE recently enough
    if (time_to_die < now) {
      log_notice(LD_GENERAL,"Not enough KEEPALIVEs, exiting cleanly.");
      tor_cleanup();
      exit(0);
      return;
    }
...
second_elapsed_callback(int fd, short event, void *args)
{
  ...
  size_t bytes_read;
  size_t relayed_bytes_written;
  size_t relayed_bytes_read;
  int seconds_elapsed;
  or_options_t *options = get_options();
  (void)fd;
  (void)event;
  ...
  relayed_bytes_written = stats_prev_relayed_write_bucket - global_relayed_write_bucket;
  relayed_bytes_read = stats_prev_relayed_read_bucket - global_relayed_read_bucket;
  seconds_elapsed = current_second ? (int)(now - current_second) : 0;
  stats_n_bytes_read += bytes_read;
  stats_n_bytes_written += bytes_written;
  if (accounting_is_enabled(options) && seconds_elapsed >= 0)
    accounting_add_bytes(bytes_read, bytes_written, seconds_elapsed);
  control_event_bandwidth_used((uint32_t)relayed_bytes_read,(uint32_t)relayed_bytes_written);
  control_event_stream_bandwidth_used();
  control_event_circuit_token_levels();

  if (seconds_elapsed > 0)
    connection_bucket_refill(seconds_elapsed, now);
  stats_prev_global_read_bucket = global_read_bucket;
  stats_prev_global_write_bucket = global_write_bucket;
  stats_prev_relayed_read_bucket = global_relayed_read_bucket;
  stats_prev_relayed_write_bucket = global_relayed_write_bucket;
