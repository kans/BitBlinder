...
#define EVENT_ORCIRCUIT        0x0017
#define EVENT_TOKEN_LEVELS     0x0018
...
static int handle_control_sendpayment(control_connection_t *conn,
                                            uint32_t len, const char *body);
static int handle_control_addtokens(control_connection_t *conn,
                                            uint32_t len, const char *body);
static int handle_control_keepalive(control_connection_t *conn,
                                            uint32_t len, const char *body);
static int handle_control_convertcircid(control_connection_t *conn,
                                            uint32_t len, const char *body);
  ...
  SMARTLIST_FOREACH_BEGIN(events, const char *, ev)
    {
      ...
      else if (!strcasecmp(ev, "TOKEN_LEVELS")) 
        event_code = EVENT_TOKEN_LEVELS;
      ...
      else if (!strcasecmp(ev, "ORCIRCUIT"))
        event_code = EVENT_ORCIRCUIT;
      else {
...
//Josh:
/** Called when we get a KEEPALIVE message. */
static int
handle_control_keepalive(control_connection_t *conn,
                                 uint32_t len, const char *body)
{
  smartlist_t *args;
  time_t extraTime;
  ...
  log_debug(LD_CONTROL, "Josh:  keepalive");

  args = getargs_helper("KEEPALIVE", conn, body, 1, 1);
  if (!args)
    goto done;

  extraTime = (time_t) tor_parse_ulong(smartlist_get(args,0), 10, 0, UINT32_MAX, &ok, NULL);
  if (!ok) {
    connection_printf_to_buf(conn, "552 Improper keepalive time: \"%s\"\r\n",
                            (char*)smartlist_get(args, 0));
    goto done;
  }
  now = time(NULL);
  time_to_die = now + extraTime;
...
//Josh:  I'm not 100% sure about this code.  Also it's not optimized.  I know there is 
//some hash table, and this function:  circuit_get_by_circid_orconn, but sometimes there
//are multiple orconns to a single router and I dont understand why
circuit_t*
find_circ_by_id_and_digest_helper(circid_t nextCircId, const char* digest) {
  circuit_t* circ = NULL;
  for (circ=global_circuitlist;circ;circ = circ->next) {
    if (! CIRCUIT_IS_ORIGIN(circ)) {
      or_circuit_t *or_circ = TO_OR_CIRCUIT(circ);
      if (or_circ->p_conn && !memcmp(or_circ->p_conn->identity_digest, digest, DIGEST_LEN)) {
        if (or_circ->p_circ_id == nextCircId) {
          return circ;
        }
        if (circ->n_circ_id == nextCircId) {
          return circ;
        }
      }
    }
    if (circ->n_conn && !memcmp(circ->n_conn->identity_digest, digest, DIGEST_LEN)) {
      if (circ->n_circ_id == nextCircId) {
        return circ;
      }
    }
...
//Josh:
/** Called when we get a SENDPAYMENT message. */
//pass:  relay circuitType circId message
//where:
//relay = hexid of a remote relay involved in the connection.  Used to get the orconnection.
//circId = the circuit id.  Defines the direction to send on the orconnection.
//message = base64-encoded bytes to send
//numHops = how many hops along the circuit to send it.  Defaults to 1.
//sendOverCircuit = whether to send over the circuit (1) or via OR connections (0).  Defaults to 0.
static int
handle_control_sendpayment(control_connection_t *conn,
                                 uint32_t len, const char *body)
{
  or_connection_t* orconn = NULL;
  uint16_t numHops;
  uint16_t sendOverCircuit;
  int dataLen;
  int ok;
  char digest[20];
  char* hexDigest;
  ...
  log_debug(LD_CONTROL, "Josh:  Sending payment...");

  args = getargs_helper("SENDPAYMENT", conn, body, 3, 5);
  if (!args)
    goto done;

  hexDigest = (char*)smartlist_get(args, 0);
  if (base16_decode(digest, 20, hexDigest, 40) < 0) {
    connection_printf_to_buf(conn, "552 Improper hex digest \"%s\"\r\n",
                            (char*)smartlist_get(args, 0));
    goto done;
  }
  
  nextCircId = (uint16_t) tor_parse_ulong(smartlist_get(args,1), 10, 0, UINT32_MAX, &ok, NULL);
  if (!ok) {
    connection_printf_to_buf(conn, "552 Improper circuit id \"%s\"\r\n",
                            (char*)smartlist_get(args, 1));
    goto done;
  }
  
  circ = find_circ_by_id_and_digest_helper(nextCircId, digest);
  if (!circ) {
    connection_printf_to_buf(conn, "552 Cannot find circuit with id \"%s\"\r\n",
                            (char*)smartlist_get(args, 1));
    goto done;
  }
  if (circ->marked_for_close) {
    connection_printf_to_buf(conn, "552 Circuit \"%s\" is closed\r\n",
                            (char*)smartlist_get(args, 1));
    goto done;
  }
  ...
  if (circ->n_circ_id == nextCircId) {
    ...
    orconn = TO_OR_CIRCUIT(circ)->p_conn;
  }
  if (!orconn) {
    connection_printf_to_buf(conn, "552 Circuit with id \"%s\" doesnt go anywhere?\r\n",
                            (char*)smartlist_get(args, 1));
    goto done;
  }
  
  if (smartlist_len(args) > 3) {
    numHops = (uint16_t) tor_parse_ulong(smartlist_get(args,3), 10, 0, UINT32_MAX, &ok, NULL);
    if (!ok) {
      connection_printf_to_buf(conn, "552 Improper number of hops \"%s\"\r\n",
  ...
  if (smartlist_len(args) > 4) {
    sendOverCircuit = (uint16_t) tor_parse_ulong(smartlist_get(args, 4), 10, 0, 1, &ok, NULL);
    if (!ok) {
      connection_printf_to_buf(conn, "552 sendOverCircuit must be 0 or 1: \"%s\"\r\n",
  ...
  encodedData = (char*)smartlist_get(args, 2);
  
  //Send a message through the direct OR connection
  if (!sendOverCircuit) {
    cell_t cell;
    //errorcheck the base64 input
    dataLen = base64_decode(cell.payload + 2, CELL_PAYLOAD_SIZE - 2, encodedData, strlen(encodedData));
    if (dataLen != CELL_PAYLOAD_SIZE - 2) {
      connection_printf_to_buf(conn, "552 Data is the wrong length!\r\n");
      goto done;
    }
    //write the cell
    cell.command = CELL_PAYMENT;
    cell.circ_id = nextCircId;
    set_uint16(cell.payload, htons(numHops));
    connection_or_write_cell_to_buf(&cell, orconn);
  //Send a message through the circuit
  } else {
    char cellData[RELAY_PAYLOAD_SIZE+1];
    crypt_path_t *hop;
    //errorcheck the base64 input
    dataLen = base64_decode(cellData, RELAY_PAYLOAD_SIZE, encodedData, strlen(encodedData));
    if (dataLen != RELAY_PAYLOAD_SIZE) {
      connection_printf_to_buf(conn, "552 Data is the wrong length!\r\n");
      goto done;
    }
    //find this hop in the circuit
    if (CIRCUIT_IS_ORIGIN(circ)) {
      hop = circuit_get_cpath_hop(TO_ORIGIN_CIRCUIT(circ), numHops);
      if (!hop) {
        connection_printf_to_buf(conn, "551 Circuit doesn't have %d hops.\r\n", numHops);
        goto done;
      }
    } else {
      hop = NULL;
    }
    relay_send_command_from_edge(0, circ, RELAY_COMMAND_MESSAGE, cellData, RELAY_PAYLOAD_SIZE, hop);
  }
   
  connection_printf_to_buf(conn, "250 %d %d\r\n", circ->payedReads, circ->payedWrites);
...
//Josh:
/** Used to add tokens for PAR circuits... */
static int
handle_control_addtokens(control_connection_t *conn,
                                 uint32_t len, const char *body)
{
  or_connection_t* orconn = NULL;
  ...
  log_debug(LD_CONTROL, "Josh:  Adding tokens...");

  args = getargs_helper("ADDTOKENS", conn, body, 4, 4);
  if (!args)
    goto done;
  ...
  numReadCells = tor_parse_long(smartlist_get(args, 2), 10, -1, INT32_MAX, &ok, NULL);
  if (!ok) {
    connection_printf_to_buf(conn, "552 bad value for numReadCells \"%s\"\r\n",
                            (char*)smartlist_get(args, 2));
    goto done;
  }
  
  numWriteCells = tor_parse_long(smartlist_get(args, 3), 10, -1, INT32_MAX, &ok, NULL);
  if (!ok) {
    connection_printf_to_buf(conn, "552 bad value for numWriteCells \"%s\"\r\n",
                            (char*)smartlist_get(args, 3));
    goto done;
  }
  
  //if bytes is less than 0, that means we should close the circuit:
  if (numReadCells < 0) {
    circuit_mark_for_close(circ, END_CIRC_REASON_UNPAID);
  //if bytes is zero, dont bother doing anything, the controller just wants to know how many payments we have
  } else if (numReadCells == 0) {
  
  } else {
    //actually add the packets:
    circ->payedReads += numReadCells;
    circ->payedWrites += numWriteCells;
    circ->payedReadsAdded += numReadCells;
    circ->payedWritesAdded += numWriteCells;
    //resume writing or reading if necessary:
    if (CIRCUIT_IS_ORIGIN(circ)) {
      ...
      do {
        ...
        layer_hint = layer_hint->prev;
      } while (layer_hint != TO_ORIGIN_CIRCUIT(circ)->cpath->prev);
    } else {
      circuit_consider_sending_sendme(circ, NULL);
...
/** Called when we get a CONVERTCIRCID message. */
//pass:  circId
//where:
//circId = the circuit id to convert to a nextCircId
static int
handle_control_convertcircid(control_connection_t *conn,
                                 uint32_t len, const char *body)
{
  uint32_t circ_id;
  int ok;
  origin_circuit_t *originCirc;
  smartlist_t *args;
  (void) len; /* body is nul-terminated, so it's safe to ignore the length. */

  args = getargs_helper("CONVERTCIRCID", conn, body, 1, 1);
  if (!args)
    goto done;
  
  circ_id = (uint32_t) tor_parse_ulong(smartlist_get(args,0), 10, 0, UINT32_MAX, &ok, NULL);
  if (!ok) {
    connection_printf_to_buf(conn, "552 Improper circuit id \"%s\"\r\n",
                            (char*)smartlist_get(args, 0));
    goto done;
  ...
  originCirc = circuit_get_by_global_id(circ_id);
  if (!originCirc) {
    connection_printf_to_buf(conn, "552 Cannot find circuit with id \"%s\"\r\n",
                            (char*)smartlist_get(args, 0));
    goto done;
  ...
  connection_printf_to_buf(conn, "250 %u\r\n", TO_CIRCUIT(originCirc)->n_circ_id);
 ...
 again:
  ...
  if (!strcasecmp(conn->incoming_cmd, "SETCONF")) {
    ...
    if (handle_control_setcircuitpurpose(conn, cmd_data_len, args))
      return -1;
  } else if (!strcasecmp(conn->incoming_cmd, "SENDPAYMENT")) {
    if (handle_control_sendpayment(conn, cmd_data_len, args))
      return -1;
  } else if (!strcasecmp(conn->incoming_cmd, "KEEPALIVE")) {
    if (handle_control_keepalive(conn, cmd_data_len, args))
      return -1;
  } else if (!strcasecmp(conn->incoming_cmd, "ADDTOKENS")) {
    if (handle_control_addtokens(conn, cmd_data_len, args))
      return -1;
  } else if (!strcasecmp(conn->incoming_cmd, "CONVERTCIRCID")) {
    if (handle_control_convertcircid(conn, cmd_data_len, args))
      return -1;
...
//Josh:
/** A second or more has elapsed: tell any interested control
 * connections about circuit token levels. */
int
control_event_circuit_token_levels(void)
{
  if (EVENT_IS_INTERESTING(EVENT_TOKEN_LEVELS)) {
    circuit_t* circ = NULL;
    for (circ=global_circuitlist;circ;circ = circ->next) {
      if (circ->marked_for_close == 0 && CIRCUIT_IS_ORIGIN(circ)) {
        origin_circuit_t *orig_circ = TO_ORIGIN_CIRCUIT(circ);

        send_control_event(EVENT_TOKEN_LEVELS, ALL_FORMATS,
                           "650 TOKEN_LEVELS %lu %ld %ld %ld %ld\r\n",
                           (unsigned long)orig_circ->global_identifier,
                           (long)circ->payedReads,
                           (long)circ->payedWrites,
                           (long)circ->payedReadsAdded,
                           (long)circ->payedWritesAdded);

        circ->payedReadsAdded = circ->payedWritesAdded = 0;
      }
    }
  }
...
//Josh:  inform the controller about an event related to an OR Conn.  (either payment or close)
void
control_event_orcircuit(const char* msgType, const char *data, circuit_t* circ, int direction)
{
  char prevHexId[HEX_DIGEST_LEN+1];
  char nextHexId[HEX_DIGEST_LEN+1];
  char* pdigest;
  char* ndigest;
  circid_t prevCircId;
  circid_t nextCircId;
  ...
  prevHexId[0] = '0';
  prevHexId[1] = '\0';
  prevHexId[HEX_DIGEST_LEN] = '\0';
  nextHexId[HEX_DIGEST_LEN] = '\0';

  pdigest = NULL;
  ndigest = NULL;
  
  if (direction == CELL_DIRECTION_OUT) {
    prevCircId = TO_OR_CIRCUIT(circ)->p_circ_id;
    if (!CIRCUIT_IS_ORIGIN(circ) && TO_OR_CIRCUIT(circ)->p_conn) {
      pdigest = TO_OR_CIRCUIT(circ)->p_conn->identity_digest;
    }
    if (circ->n_conn) {
      nextCircId = circ->n_circ_id;
      ndigest = circ->n_conn->identity_digest;
    }
  } else {
    if (circ->n_conn) {
      prevCircId = circ->n_circ_id;
      pdigest = circ->n_conn->identity_digest;
    }
    if (!CIRCUIT_IS_ORIGIN(circ) && TO_OR_CIRCUIT(circ)->p_conn) {
      nextCircId = TO_OR_CIRCUIT(circ)->p_circ_id;
      ndigest = TO_OR_CIRCUIT(circ)->p_conn->identity_digest;
    }
  }
  if (ndigest) {
    base16_encode(nextHexId, HEX_DIGEST_LEN+1, ndigest, DIGEST_LEN);
  }
  if (pdigest) {
    base16_encode(prevHexId, HEX_DIGEST_LEN+1, pdigest, DIGEST_LEN);
  }
  
  send_control_event(EVENT_ORCIRCUIT, ALL_FORMATS,
    "650 ORCIRCUIT %s %u %s %u %s %s\r\n", msgType, prevCircId, prevHexId, nextCircId, nextHexId, data);
}
