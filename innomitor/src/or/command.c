...
static void command_process_payment_cell(cell_t *cell, or_connection_t *conn);
...
#define PROCESS_CELL(tp, cl, cn) command_process_ ## tp ## _cell(cl, cn)
  ...
  switch (cell->command) {
    case CELL_PADDING:
      ...
      break;
    case CELL_PAYMENT:
      PROCESS_CELL(payment, cell, conn);
      break;
    case CELL_CREATE:
...
//Josh:  handle payment cell
/** Process a 'payment' <b>cell</b> that just arrived from <b>conn</b>. Just
 * inform the controller of the contents of the cell
 */
static void
command_process_payment_cell(cell_t *cell, or_connection_t *conn)
{        
  circuit_t *circ;
  circid_t nextCircId;
  or_connection_t *nextConn;
  int direction;
  uint16_t hopNum;
  char base64Buffer[1000];
  
  log_debug(LD_OR, "Josh:  Received payment cell!");
  ...
  if (!CIRCUIT_IS_ORIGIN(circ) &&
    ...
    if (circ->n_conn) {
      ...
      nextConn = circ->n_conn;
    }
  } else {
    direction = CELL_DIRECTION_IN;
    if (!CIRCUIT_IS_ORIGIN(circ) && TO_OR_CIRCUIT(circ)->p_conn) {
      nextCircId = TO_OR_CIRCUIT(circ)->p_circ_id;
      nextConn = TO_OR_CIRCUIT(circ)->p_conn;
    }
  }

  //if the cell should be forwarded on:
  hopNum = ntohs(get_uint16(cell->payload)) - 1;
  if (hopNum >= 1 && nextConn) {
    cell->command = CELL_PAYMENT;
    cell->circ_id = nextCircId;
    set_uint16(cell->payload, htons(hopNum));
    connection_or_write_cell_to_buf(cell, nextConn);
  } else {
    //have to convert data to base64 to get it through the control interface
    if (base64_encode(base64Buffer, 1000, cell->payload+2, CELL_PAYLOAD_SIZE-2) < 0) {
      log_warn(LD_CONTROL, "Josh:  Unable to format string for controller.");
      return;
    }
    tor_strstrip(base64Buffer, "\n");
    
    control_event_orcircuit("PAYMENT", base64Buffer, circ, direction);
  }
}
