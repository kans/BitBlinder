 ...
 * the directory. */
 //Josh:  changed this because we have a much shorter consensus update period.  Would be nice to have the network respond more quickly to up and down routers.
//#define REACHABLE_TIMEOUT (45*60)
#define REACHABLE_TIMEOUT (35*60)
...
//  time_t cutoff = now - ROUTER_MAX_AGE_TO_PUBLISH;
  routerlist_t *rl = router_get_routerlist();
  static char ctr = 0;
  //Josh:  Note:  I changed this from 128 to 64 to make reachability testing happen more often, since we also changed REACHABLE_TIMEOUT
  int testSubsets = 64;
  int bridge_auth = authdir_mode_bridge(get_options());
...
//      continue;
    ...
    if (try_all || (((uint8_t)id_digest[0]) % testSubsets) == ctr) {
